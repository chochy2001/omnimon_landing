...
present "upload retries transient network errors" "--retry-all-errors"
present "upload keeps the shared identity bytes" "scripts/ftps_identity.sh"
present "png structural identity branch" "png-structural-identity"
present "asset gate sources the png comparator" "scripts/png_compare.sh"
present "dotfiles must not be served" "dotfiles-are-not-served"
